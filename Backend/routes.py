...
# MVP: Manager Views Completed Requests
    ...
    def allocate_asset(asset_id):
        ...
        data = request.get_json()
        username = data.get('username')  # Use 'username' directly
        user = User.query.filter_by(username=username).first()  # Query user by username directly
        if not user:
        ...
        return jsonify({'message': 'Asset allocated to user successfully'}), 201
